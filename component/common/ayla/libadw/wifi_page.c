...
static xSemaphoreHandle wcw_sema;
 ...
 *
 * Note the device AP mode address (<ap-mode-addr>) is from <adw/wifi_conf.h>.
 *
 * IOS 7                Ayla
 ...
 *  If has Ayla app:
 * GET some_ip_addr/junk by CaptiveNetwork
 *                      redir <ap-mode-addr>/library/test/success
 ...
 * GET /library/test/success
 *                      success
 * GET <ap-mode-addr>/wifi by iphone OS 7
 *                      /
 *
 * IOS 6
...
{
	xSemaphoreTake(wcw_sema, portMAX_DELAY);
	wsw->wsw_req = req;
	wsw->wsw_next = wsw_head;
	wsw_head = wsw;
	req->close_cb = adw_wifi_scan_wait_dequeue;
	xSemaphoreGive(wcw_sema);
}
...
{
	char hdr_msg[120];
	ip_addr_t ip_addr = {.addr = ADW_WIFI_AP_IP};
	char assign[80];
	char ip_str[20];
	...
	snprintf(assign, sizeof(assign),
	    "window.location.assign(\"http://%s/wifi\");",
	    adw_format_ip(&ip_addr, ip_str, sizeof(ip_str)));
	...
	if (req->ios_ver > 10) {
		server_put(req,
		    ...
		    "</script></body></html>\n",
		    adw_wifi_ios_app, conf_sys_dev_id, assign);
	} else {
		server_put(req,
		    "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN "
		    "http://www.w3.org/TR/html4/strict.dtd\">\n"
		    ...
		    "window.onload=wifi_setup;\n"
		    "</script></body></html>\n",
		    assign, adw_wifi_ios_app, conf_sys_dev_id, assign);
	}
}
	...
	u8 hidden)
{
	...
	adw_log(LOG_DEBUG "add_prof ssid %s sec %u %s%s",
	    adw_format_ssid(ssid, ssid_buf,
	    sizeof(ssid_buf)), sec_token, mfi ? "mfi " : "",
	    hidden ? "hidden " : "");
...
const char *adw_format_ssid(const struct adw_ssid *ssid, char *buf, size_t len)
{
...
void adw_wifi_page_init(int enable_redirect)
{
	...
	vSemaphoreCreateBinary(wcw_sema);
}
