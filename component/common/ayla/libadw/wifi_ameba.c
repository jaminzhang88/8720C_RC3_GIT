...
enum wifi_error adw_wmi_join(struct adw_state *wifi, struct adw_profile *prof)
{
	...
	if (rc != RTW_SUCCESS) {
		...
		last_err = wifi_get_last_error();
		adw_log(LOG_DEBUG
		    "adw_wmi_join: last_err %d", last_err);
		switch (last_err) {
		case  RTW_NONE_NETWORK:
			wifi_err = WIFI_ERR_NOT_FOUND;
			break;
		case  RTW_CONNECT_FAIL:
			wifi_err = WIFI_ERR_TIME;
			break;
		case  RTW_WRONG_PASSWORD:
			wifi_err = WIFI_ERR_WRONG_KEY;
			break;
		case  RTW_4WAY_HANDSHAKE_TIMEOUT:
			wifi_err = WIFI_ERR_TIME;
			break;
		case  RTW_DHCP_FAIL:
			wifi_err = WIFI_ERR_NO_IP;
			break;
		default:
			wifi_err = WIFI_ERR_MEM;
			break;
		}
		return wifi_err;
	}
...
int adw_wmi_dhcp_poll(struct adw_state *wifi, struct adw_wifi_history *hist)
{
	...
	for (i = 0; i < min(DNS_MAX_SERVERS, WIFI_HIST_DNS_SERVERS); i++) {
		hist->dns_servers[i] = dns_getserver_adapt(i);
	}
	return 0;
}
