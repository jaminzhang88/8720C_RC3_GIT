    ...
    char *buf, size_t len)
{
	...
	if (httpd_request_get_query_key(conn, (char *)name, &result)) {
		return NULL;
	}
...
static int server_send(struct server_req *req, const char *msg, size_t len)
{
	...
	return httpd_response_write_data(conn, (uint8_t *)msg, len) != len;
}
...
static void server_flush(struct server_req *req, const char *msg)
{
	...
	if (httpd_response_write_data(conn, (uint8_t *)msg, req->len) != req->len) {
		req->err = AE_ERR;
		goto out;
	}
...
enum server_method server_req_method(struct server_req *req)
{
	...
	for (method = REQ_BAD; method < ARRAY_LEN(server_method_names);
		...
		if (httpd_request_is_method(conn,
		    (char *)server_method_names[method])) {
			return method;
		}
	}
	return REQ_BAD;
	...
	char *buf, size_t size)
{
	...
	rc = httpd_request_get_header_field(conn, (char *)header, &value);
	if (rc == 0) {
...
void server_reg_urls(const struct url_list *list)
{
	...
	for (item = list; item->url; item++) {
		...
		httpd_reg_page_callback((char *)item->url, server_handler);
	}
}
