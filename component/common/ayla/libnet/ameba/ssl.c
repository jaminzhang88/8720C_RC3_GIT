...
enum ada_err stream_close(struct stream_pcb *sp)
{
	...
	/* Yield control to stream thread to clean up */
	vTaskDelay(100);
	return AE_OK;
}
...
recv_more:
	...
	if (sp->enable) {
		...
		len = mbedtls_ssl_read(&sp->ssl, ada_mbuf_payload(m),
		    STREAM_RX_SZ);
		stream_stack_check(sp);
		avail_len = mbedtls_ssl_get_bytes_avail(&sp->ssl);
		ada_lock(stream_lock);
...
{
	stream_lock = (struct ada_lock *)ada_lock_create("stream");
	ASSERT(stream_lock);
}
...
static enum ada_err stream_tcp_connected(struct stream_pcb *sp)
{
	...
	if (sp->enable) {
		...
		if (ca_certs_inited) {
			...
			mbedtls_ssl_conf_verify_adapt(&sp->ssl_cfg,
