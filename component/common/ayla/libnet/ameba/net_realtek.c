...
void net_init(void)
{
	...
	TAILQ_INIT(&net_pcbs);
	net_pcb_lock = (struct ada_lock *)ada_lock_create("net_pcbs");
	ASSERT(net_pcb_lock);
	...
	net_timer_lock = (struct ada_lock *)ada_lock_create("net_timer_lock");
...
enum ada_err net_dns_lookup(struct net_dns_req *req)
{
	...
	net_callback_init1(&req->net_dns_cb, net_dns_helper, req);
	req->addr = 0;
...
struct net_tcp *net_tcp_alloc_set(int s)
{
	...
	tcp = (struct net_tcp *)ayla_calloc(1, sizeof(*tcp));
	if (!tcp) {
		return NULL;
	}
